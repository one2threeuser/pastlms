...
from course.models import VideoWatchProgress, UploadVideo
...
def quiz_list(request, slug):
    ...
    videos = UploadVideo.objects.filter(course=course)  # Assuming course has related videos via UploadVideo
    user = request.user

    # Initialize a dictionary to store the watched status for each video
    video_progress = {video.id: False for video in videos}

    # Fetch the videos that the user has watched
    watched_videos = VideoWatchProgress.objects.filter(user=user, video__in=videos, watched=True)
    
    # Mark watched videos as True in the video_progress dictionary
    for progress in watched_videos:
        video_progress[progress.video.id] = True  # Set watched video status to True

    # Check if all videos are watched
    all_videos_watched = all(video_progress.values())  # True if all videos are watched, else False
    ...
    return render(
        ...
        "quiz/quiz_list.html",
        {
            "quizzes": quizzes,
            "course": course,
            "video_progress": video_progress,  # Pass the video progress to the template
            "all_videos_watched": all_videos_watched,  # Flag to indicate whether all videos are watched
        },
    )
    ...
    # # Unlock quiz that have been completed
    # for course in course:
    #     if course.id in completed_courses:
    #         course_progress[course.id] = True

    # return render(
    #     request, "quiz/quiz_list.html", {"quizzes": quizzes, "course": courses, "course_progress": course_progress,}
    # )
...
class QuizTake(FormView):
    ...
    def dispatch(self, request, *args, **kwargs):
        ...
        # NEW
        # Check if user has watched all videos in the course
        videos = UploadVideo.objects.filter(course=self.course)
        user = request.user
        video_progress = {video.id: False for video in videos}
        watched_videos = VideoWatchProgress.objects.filter(user=user, video__in=videos, watched=True)
        ...
        all_videos_watched = all(video_progress.values())

        # If not all videos are watched, lock the quiz
        if not all_videos_watched and request.user.is_student:
            messages.warning(request, "Quiz Locked: You need to watch all videos before attempting the quiz.")
            return redirect("quiz_index", self.course.slug)

        # Check if the quiz is in draft mode and user doesn't have permission
