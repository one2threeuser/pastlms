    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.success(
                ...
                + " "
                + last_name
                + " has been created. Please activate this user by sending the activation email."
            )
...
@login_required
@admin_required
def send_activation_email(request, pk):
    """
    Manually send an activation email to a student or lecturer.
    """
    user = get_object_or_404(User, pk=pk)

    # if user.is_active:
    #     messages.warning(request, "This account is already activated.")
    #     return redirect("student_list" if user.is_student else "lecturer_list")
    ...
    send_new_account_email(user, password)  # ✅ Uses the `utils.py` function

    if user.is_active:
        messages.warning(request, "This account is already activated. Activation email resend.")
        return redirect("student_list" if user.is_student else "lecturer_list")
    else:
        user.is_active = True
        user.save()
